...
/**
 * @title FBTCAdapter
 * @notice Function fBTC adapter for MOSAIC. Uses Merchant Moe LB v2.2.
 */
contract FBTCAdapter is IProtocolAdapter, Ownable, ReentrancyGuard {
    ...
    address public immutable TARGET; // fBTC address
    ...
    function priceUSDC() external view override returns (uint256) {
        uint256 oneUnit = 10 ** IERC20D(TARGET).decimals();
        address[] memory route = new address[](2);
        route[0] = TARGET;
        route[1] = USDC;
        IMoeLBQuoter.Quote memory q = IMoeLBQuoter(MOE_LB_QUOTER).findBestPathFromAmountIn(
